 ...
 */
export const fetchCodelabsPageData =
  async (): Promise<CodelabsPageResponse> => {
    ...
    try {
      ...
      if (!docSnapshot.exists) {
        const errorMessage = "Codelabs data not found in database";
      ...
      const codelabs: Codelab[] = Array.isArray(data?.codelabs)
        ? data.codelabs
        : [];
      ...
      throw new Error(
        "Failed to fetch codelabs data from the database: " + typedError.message
      );
    }
  };
